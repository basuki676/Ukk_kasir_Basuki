...
class DashboardController extends Controller
{
    public function ViewDashboard()
    {
        ...
        if (auth()->user()->role == 'employe') {
            ...
            $data['lastUpdated'] = $lastSale ? $lastSale->created_at->format('d M Y H:i') : 'Belum ada transaksi hari ini';
            ...
            $change = $yesterdaySales ? round((($todaySales - $yesterdaySales) / $yesterdaySales) * 100, 2) : 0;
            ...
            $chartRawData = Sale::select(DB::raw('DATE(created_at) as date'), DB::raw('COUNT(*) as total'))
            ...
            $products = Product::withCount([
                'detailSales as sales_count' => function ($query) use ($startDate, $endDate) {
                    $query->select(DB::raw('COALESCE(SUM(amount), 0)'))->whereBetween('created_at', [$startDate, $endDate]);
                },
            ])
